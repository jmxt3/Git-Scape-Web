...
const App: React.FC = () => {
  ...
  const [userProvidedGeminiApiKey, setUserProvidedGeminiApiKey] = useState<string | null>(null);
  const [showGeminiApiModal, setShowGeminiApiModal] = useState<boolean>(false);

  const [processedRepoName, setProcessedRepoName] = useState<string | undefined>(undefined);
  const [repoNameForFilename, setRepoNameForFilename] = useState<string | null>(null);
  const [filesAnalyzedCount, setFilesAnalyzedCount] = useState<number | null>(null);
  const [currentDefaultBranch, setCurrentDefaultBranch] = useState<string | null>(null);
  const [filesToRenderInDiagram, setFilesToRenderInDiagram] = useState<GithubFile[]>([]);

  const [showDiagramFullscreenModal, setShowDiagramFullscreenModal] = useState<boolean>(false);
  const [diagramDataForModal, setDiagramDataForModal] = useState<RawDiagramNode | null>(null);
  const [repoNameForModal, setRepoNameForModal] = useState<string | undefined>(undefined);
  const [defaultBranchForModal, setDefaultBranchForModal] = useState<string | null>(null);

  const websocketRef = useRef<WebSocket | null>(null);
  const wsConnectionAttemptNumberRef = useRef<number>(0);
  const currentRepoInfoRef = useRef<{owner: string, repo: string} | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      const storedGeminiKey = localStorage.getItem(GEMINI_API_KEY_LOCAL_STORAGE_KEY);
      if (storedGeminiKey) {
  ...
  const processSuccessfulDigestData = useCallback(async (
    ...
    if (branchToUse && githubService) {
        try {
            diagramFiles = await githubService.getRepoFileTree(owner, repo, branchToUse);
            ...
            console.error(`Error fetching repository structure for diagram:`, diagramErr);
        }
    } else {
  ...
  const handleGenerateDigest = useCallback(async () => {
    ...
    try {
      ...
      setError(`Failed to fetch repository details (branch): ${branchError.message}. Please ensure the repository is public or a valid GitHub token is provided for private repositories.`);
      setIsLoading(false);
    ...
    const initiateWebSocketConnection = () => {
        ...
        if (attemptNumber > 1) {
            setProgressMessage(`Connection attempt ${attemptNumber-1} failed. Retrying connection (attempt ${attemptNumber})...`);
        } else {
            setProgressMessage('Connecting to server for processing...');
        }

        const wsHost = "git-scape-api-678713936945.us-west1.run.app";
        let wsScheme: string;
        if (wsHost === "git-scape-api-678713936945.us-west1.run.app") {
          wsScheme = "wss";
        } else {
        ...
        if (githubToken) {
          wsUrl.searchParams.append('github_token', encodeURIComponent(githubToken));
        }
        ...
        try {
          ...
          ws.onmessage = async (event) => {
            ...
            try {
                ...
                switch (parsedMessageData.type) {
                    case 'progress':
                        setProgressMessage(parsedMessageData.message || 'Processing...');
                        if (typeof parsedMessageData.percentage === 'number') {
                            setProgressPercent(prev => Math.max(prev, Number(parsedMessageData.percentage)));
                        } else {
                            console.warn("Progress JSON message received without a 'percentage' number:", parsedMessageData);
                        }
                        break;
                    case 'final_digest':
                    case 'digest_complete':
                        ...
                        break;
                    case 'error':
                        const errorMessage = parsedMessageData.message || 'An error occurred during processing on the server.';
                        console.error('Error message from server (via WebSocket JSON):', errorMessage);
          ...
          const handleFailedConnectionAttempt = (errorType: string, eventDetails: any) => {
              console.error(`WebSocket connection attempt ${attemptNumber} ${errorType}:`, eventDetails);
              if (!digest) { // Only retry if no digest has been successfully processed
                  if (attemptNumber < MAX_TOTAL_WEBSOCKET_ATTEMPTS) {
                      initiateWebSocketConnection(); // Retry
                  } else {
                      setError("We couldn't fetch the repository. Please add a GitHub Personal Access Token (PAT) and try again.");
          ...
          ws.onclose = (event) => {
            ...
            if (!event.wasClean && !digest && isLoading) { // Check isLoading to ensure this is for current request
                ...
                if (attemptNumber >= MAX_TOTAL_WEBSOCKET_ATTEMPTS) {
                    setError("We couldn't fetch the repository. Please add a GitHub Personal Access Token (PAT) and try again.");
                    setProgressMessage('Connection closed unexpectedly after all attempts.');
                    setIsLoading(false);
                    setProgressPercent(0);
                } else {
          ...
          if (attemptNumber < MAX_TOTAL_WEBSOCKET_ATTEMPTS) {
            ...
            setError(err.message || "We couldn't fetch the repository. Please add a GitHub Personal Access Token (PAT) and try again.");
  ...
  }, [repoUrl, githubToken, githubService, isLoading, error, digest, progressPercent, processSuccessfulDigestData, setProcessedRepoName, setRepoNameForFilename, setFilesAnalyzedCount, setCurrentDefaultBranch, setFilesToRenderInDiagram, setDigest, setError, setIsLoading, setProgressMessage, setProgressPercent]);
  ...
  const diagramData: RawDiagramNode | null = useMemo(() => {
    if (processedRepoName && filesToRenderInDiagram.length > 0) {
      return transformGithubTreeToD3Hierarchy(filesToRenderInDiagram, processedRepoName);
    }
    return null;
  ...
  const handleOpenDiagramFullscreenModal = useCallback((data: RawDiagramNode, repoNameModal: string, branch: string | null) => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 flex flex-col">
      ...
      <div className="mb-6 mt-6">
        <div className="relative w-full max-w-4xl mx-auto flex sm:flex-row flex-col justify-center items-start sm:items-center pt-8 sm:pt-0">
            <svg className="h-auto w-16 sm:w-20 md:w-24 flex-shrink-0 p-2 md:relative sm:absolute lg:absolute left-0 lg:-translate-x-full md:translate-x-10 sm:-translate-y-16 md:-translate-y-0 -translate-x-2 lg:-translate-y-10" viewBox="0 0 91 98" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M79.653 5.729c-2.436 5.323-9.515 15.25-18.341 12.374m9.197 16.336c2.6-5.851 10.008-16.834 18.842-13.956m-9.738-15.07c-.374 3.787 1.076 12.078 9.869 14.943M70.61 34.6c.503-4.21-.69-13.346-9.49-16.214M14.922 65.967c1.338 5.677 6.372 16.756 15.808 15.659M18.21 95.832c-1.392-6.226-6.54-18.404-15.984-17.305m12.85-12.892c-.41 3.771-3.576 11.588-12.968 12.681M18.025 96c.367-4.21 3.453-12.905 12.854-14" stroke="#000" strokeWidth="2.548" strokeLinecap="round">
                </path>
            </svg>
            ...
            <svg className="w-16 lg:w-20 h-auto lg:absolute flex-shrink-0 right-0 bottom-0 md:block hidden translate-y-10 md:translate-y-20 lg:translate-y-4 lg:translate-x-full -translate-x-10" viewBox="0 0 92 80" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M75.062 40.108c1.07 5.255 1.072 16.52-7.472 19.54m7.422-19.682c1.836 2.965 7.643 8.14 16.187 5.121-8.544 3.02-8.207 15.23-6.971 20.957-1.97-3.343-8.044-9.274-16.588-6.254M12.054 28.012c1.34-5.22 6.126-15.4 14.554-14.369M12.035 28.162c-.274-3.487-2.93-10.719-11.358-11.75C9.104 17.443 14.013 6.262 15.414.542c.226 3.888 2.784 11.92 11.212 12.95" stroke="#000" strokeWidth="2.319" strokeLinecap="round">
                </path>
            </svg>
        </div>
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto px-4">
          ...
          <div className="bg-slate-800/60 backdrop-blur-md p-6 rounded-xl shadow-xl border border-slate-700/80 hover:border-slate-600 transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl">
            ...
            <p className="text-sm text-slate-300 leading-relaxed">Explore interactive, zoomable diagrams of your GitHub repository structures.</p>
          </div>
          ...
          <div className="bg-slate-800/60 backdrop-blur-md p-6 rounded-xl shadow-xl border border-slate-700/80 hover:border-slate-600 transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl">
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-xl font-semibold text-sky-400">Code Assistant</h3>
            </div>
            <p className="text-sm text-slate-300 leading-relaxed">Instantly explain, debug, refactor, and discuss code with an AI that understands your repository.</p>
          </div>
        </div>
      </div>
      <main className="container mx-auto px-4 py-8 flex-grow max-w-4xl">
        <div className="space-y-12">
          <section id="digest-generator-input" className="bg-slate-800/80 backdrop-blur-sm p-6 rounded-lg shadow-xl border border-slate-700">
            {isLoading && progressPercent > 0 && (
              <div className="w-full bg-slate-600 rounded-full h-2.5 mb-3 overflow-hidden" aria-live="polite">
                <div
      ...
      {showDiagramFullscreenModal && diagramDataForModal && repoNameForModal && (
